...
if (isset($_GET['penentuan_id'])) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        $lap_niuji_file = $_FILES['lap_niuji'];

        // Jika file diunggah
        if ($lap_niuji_file['error'] === UPLOAD_ERR_OK) {
            // Validasi file
            $allowedExtensions = ['pdf', 'doc', 'docx'];
            $fileExtension = strtolower(pathinfo($lap_niuji_file['name'], PATHINFO_EXTENSION));
            ...
            if (!in_array($fileExtension, $allowedExtensions)) {
                ...
                $fileName = basename($lap_niuji_file['name']);
                $uploadPath = $uploadDir . $fileName;

                if (file_exists($uploadPath)) {
                    echo "<script>alert('File dengan nama yang sama sudah ada. Ganti nama file dan coba lagi.');</script>";
                } else {
                    if (move_uploaded_file($lap_niuji_file['tmp_name'], $uploadPath)) {
                        // Update `lap_niuji` di database
                        $updatequery = "UPDATE penentuan SET lap_niuji = '$fileName' WHERE id = '$penentuan_id'";
                ...
                <div class="card-body">
                    <form method="POST" enctype="multipart/form-data">
                        <div class="form-group">
                            <label for="lap_niuji">Upload Laporan</label>
                            <input type="file" name="lap_niuji" id="lap_niuji" class="form-control" required>
