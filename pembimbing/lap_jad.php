...
if (isset($_GET['penentuan_id'])) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        $lap_jadbim_file = $_FILES['lap_jadbim'];

        // Jika file diunggah
        if ($lap_jadbim_file['error'] === UPLOAD_ERR_OK) {
            // Validasi file
            $allowedExtensions = ['pdf', 'doc', 'docx'];
            $fileExtension = strtolower(pathinfo($lap_jadbim_file['name'], PATHINFO_EXTENSION));
            ...
            if (!in_array($fileExtension, $allowedExtensions)) {
                ...
                $fileName = basename($lap_jadbim_file['name']);
                $uploadPath = $uploadDir . $fileName;

                if (file_exists($uploadPath)) {
                    echo "<script>alert('File dengan nama yang sama sudah ada. Ganti nama file dan coba lagi.');</script>";
                } else {
                    if (move_uploaded_file($lap_jadbim_file['tmp_name'], $uploadPath)) {
                        // Update `lap_jadbim` di database
                        $updatequery = "UPDATE penentuan SET lap_jadbim = '$fileName' WHERE id = '$penentuan_id'";
                ...
                <div class="card-body">
                    <form method="POST" enctype="multipart/form-data">
                        <div class="form-group">
                            <label for="lap_jadbim">Upload Laporan</label>
                            <input type="file" name="lap_jadbim" id="lap_jadbim" class="form-control" required>
