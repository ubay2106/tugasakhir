...
if (isset($_GET['penentuan_id'])) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        $lap_nibim_file = $_FILES['lap_nibim'];

        // Jika file diunggah
        if ($lap_nibim_file['error'] === UPLOAD_ERR_OK) {
            // Validasi file
            $allowedExtensions = ['pdf', 'doc', 'docx'];
            $fileExtension = strtolower(pathinfo($lap_nibim_file['name'], PATHINFO_EXTENSION));
            ...
            if (!in_array($fileExtension, $allowedExtensions)) {
                ...
                $fileName = basename($lap_nibim_file['name']);
                $uploadPath = $uploadDir . $fileName;

                if (file_exists($uploadPath)) {
                    echo "<script>alert('File dengan nama yang sama sudah ada. Ganti nama file dan coba lagi.');</script>";
                } else {
                    if (move_uploaded_file($lap_nibim_file['tmp_name'], $uploadPath)) {
                        // Update `lap_nibim` di database
                        $updatequery = "UPDATE penentuan SET lap_nibim = '$fileName' WHERE id = '$penentuan_id'";
                        
                        if (mysqli_query($conn, $updatequery)) {
                            echo "<script>
                                    alert('Laporan berhasil diunggah');
                                    window.location.href = '../pembimbing/nilai_proposal.php';
                                  </script>";
                            exit;
                        } else {
                ...
                <div class="card-body">
                    <form method="POST" enctype="multipart/form-data">
                        <div class="form-group">
                            <label for="lap_nibim">Upload Laporan</label>
                            <input type="file" name="lap_nibim" id="lap_nibim" class="form-control" required>
